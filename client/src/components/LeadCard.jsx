 ...
 *
 * UPDATED IN PART 2 PHASE 1:
 * Added "AI History" button that opens AIHistoryModal.
 * Shows message count badge if lead has AI messages.
 */
...
const LeadCard = ({ lead, onEdit, onDelete, onStatusChange }) => {
  ...
  const aiMessageCount = lead.aiMessages?.length || 0;
    ...
    <>
      <div className="card p-4 hover:shadow-card-hover transition-shadow duration-150">
        <div className="flex items-start gap-3">
          {/* Avatar */}
          <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${avatarColors[lead.status] || "bg-gray-100 text-gray-600"}`}>
            {initials}
          </div>
        ...
        <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-surface-100">
          ...
          <button
            ...
            title={`AI history (${aiMessageCount} messages)`}
            className="relative p-1.5 rounded-lg text-gray-400 hover:text-brand-600 hover:bg-brand-50 transition-all duration-100"
          >
            ...
            {aiMessageCount > 0 && (
              <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-brand-600 text-white text-[9px] font-bold flex items-center justify-center">
                {aiMessageCount > 9 ? "9+" : aiMessageCount}
              </span>
            )}
          </button>
